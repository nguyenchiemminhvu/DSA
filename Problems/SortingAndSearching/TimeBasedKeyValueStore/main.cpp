...
public:
    ...
    {
        std::vector<std::pair<int, std::string>>& timeline = m_map[key];
        ...
        if (found != -1)
        {
            res = timeline[found].second;
        }
        return res;
    }
private:
    std::unordered_map<std::string, std::vector<std::pair<int, std::string>>> m_map;
};
...
int main()
{
    ...
    cout << timeMap.get("foo", 0) << endl; // Output: ""
    cout << timeMap.get("bar", 1) << endl; // Output: ""
    return 0;
}